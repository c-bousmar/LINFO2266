...
public final class SequentialSolver<T> implements Solver {
    ...
    public void maximize() {
        ...
        // Initialize the solver with the root subproblem
        SubProblem<T> root = new SubProblem<>(problem.initialState(), problem.initialValue(), Integer.MAX_VALUE, Collections.emptySet());
        frontier.push(root);

        // Main loop to process nodes in the frontier
        while (!frontier.isEmpty()) {
            SubProblem<T> sub = frontier.pop();

            // Restrict the current problem if the UB is promising
            if (sub.getUpperBound() > bestLB) {
                int width = this.width.maximumWidth(sub.getState());
                ...
                // If not exact, process further
                if (!mdd.isExact()) {
                    // Relax and enqueue further problems
