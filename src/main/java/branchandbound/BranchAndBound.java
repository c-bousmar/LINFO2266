...
public class BranchAndBound {
    ...
    public static <T> void minimize (OpenNodes<T> openNode, Consumer<Node<T>> onSolution) {
        ...
        int iter = 0;
        double[] optimality_gap = new double[10001];

        while (!openNode.isEmpty() && iter < 10000) {
            ...
            while(!openNode.isEmpty()) {
                Node<T> temp = openNode.remove();
                globalLow = Math.min(globalLow, temp.objectiveFunction());
                tempQueue.add(temp);
            }
            for (Node<T> temp : tempQueue) {
                openNode.add(temp);
            }
            ...
            optimality_gap[iter] = (globalLow / -(upperBound - globalLow)) * 100;

            System.out.println(upperBound);
            System.out.println(globalLow);
        }
        String outputFilePath = "output.txt";
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFilePath))) {
            for (double val : optimality_gap) {
