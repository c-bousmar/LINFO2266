...
public class DynamicProgramming<S extends State> {
    ...
    private double getValueForState(S state) {
        ...
        if (table.containsKey(state)) return table.get(state);
        ...
        double state_value = model.isMaximization() ? Double.MIN_VALUE : Double.MAX_VALUE;
        for (Transition<S> transition : model.getTransitions(state)) {
            double successor_value = getValueForState(transition.getSuccessor());
            if ((model.isMaximization() && successor_value + transition.getValue() > state_value) ||
                    (!model.isMaximization() && successor_value + transition.getValue() < state_value))
                state_value = successor_value + transition.getValue();
        }
        table.put(state, state_value);
        return state_value;
    }
    ...
    private Solution rebuildSolution() {
        // TODO rebuild solution by traversing the tableau
        S current_state = model.getRootState();
        List<Integer> path = new LinkedList<>();
        while (!model.isBaseCase(current_state)) {
            for (Transition<S> transition : model.getTransitions(current_state)) {
                if (table.get(transition.getSuccessor()) + transition.getValue() == table.get(current_state)) {
                    path.add(transition.getDecision());
                    current_state = transition.getSuccessor();
                    break;
                }
            }
        }
        return new Solution(table.get(model.getRootState()), path);
    }
}
