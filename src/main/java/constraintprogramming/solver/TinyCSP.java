...
public class TinyCSP {
    ...
    public AtomicInteger nRecurAtom;
    public int nRecur = 0;
    ...
    public void dfs(Runnable closure, AtomicInteger failureCounter) {
        nRecurAtom.incrementAndGet();

        Optional<Variable> notFixed = firstNotFixed();
        if (!notFixed.isPresent()) {
            closure.run();
        } else {
    ...
    public void setRecursiveCallTracker(AtomicInteger counter) {
        this.nRecurAtom = counter;
    }
