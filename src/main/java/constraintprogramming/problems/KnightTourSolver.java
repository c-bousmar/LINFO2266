...
public class KnightTourSolver {
    ...
    public static List<KnightTourInstance.Solution> solve(KnightTourInstance instance) {
        ...
        for (int i = 0; i < n * n - 1; i++) {
            csp.add(new KnightMove(x[i], x[i + 1], n));
        }
        ...
        // Last position is the starting position
        csp.add(new KnightMove(x[n * n - 1], x[0], n));

        // Preset moves
        for (int step = 0; step < n * n; step++) {
            if (instance.isMove(step)) {
                x[step].dom.fix(instance.move(step));
            }
        }
